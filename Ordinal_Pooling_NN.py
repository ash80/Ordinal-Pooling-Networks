...
with graph.as_default():
    ...
    conv1_pool = tf.Variable(tf.tile(tf.constant([[0.6, 0.25, 0.1, 0.05]]), [depth, 1]))
    
    layer2_weights = tf.Variable(tf.truncated_normal(
        [patch_size, patch_size, depth, 2*depth], stddev=0.02))
    layer2_biases = tf.Variable(tf.constant(1.0, shape=[2*depth]))
    conv2_pool = tf.Variable(tf.tile(tf.constant([[0.6, 0.25, 0.1, 0.05]]), [2*depth, 1]))
    ...
    def model_pool_dropout(data, if_dropout=True):
        ...
        # Ordinal Pooling network Layer 1
        conv_stack = tf.stack([conv[:, ::2, ::2, :],
                               conv[:, 1::2, ::2, :],
                               conv[:, ::2, 1::2, :],
                               conv[:, 1::2, 1::2, :]], axis=4)
        conv_sort = tf.nn.top_k(conv_stack, k=4).values
        pooling = tf.reduce_sum(conv_sort * conv1_pool, axis=4)
        ...
        # Ordinal Pooling network Layer 2
        conv_stack = tf.stack([conv[:, ::2, ::2, :],
                               conv[:, 1::2, ::2, :],
                               conv[:, ::2, 1::2, :],
                               conv[:, 1::2, 1::2, :]], axis=4)
        conv_sort = tf.nn.top_k(conv_stack, k=4).values
        pooling = tf.reduce_sum(conv_sort * conv2_pool, axis=4)
...
with tf.Session(graph=graph) as session:
    ...
    log_file_name = 'OrdPoolNN_log.log'
    model_file_name = './OrdPoolNN_model.ckpt'
